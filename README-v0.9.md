# Storm Chaser v0.9 — Field Polish
...
- The truck now uses an actual 3D body, four rotating wheel assemblies, front-wheel steering, restrained yaw, damped vertical suspension, roof equipment and moving antennas. Original tailgate artwork is mapped onto the rigid model. The earlier shallow truck-image mesh is removed.
...
- Paint wear, scratches and shallow tailgate dents follow hull damage. A damaged taillight flickers, antennas rattle more, and the engine develops an uneven note. Repairing hull repairs the corresponding visual damage.
...
94 automated checks cover driving, safe checkpoint returns, retry snapshots, both complete mission paths, gallery state, persistent settings, assistance, wheel/suspension behavior, water pooling, and character reaction cooldowns. Rendering and exported Linux startup are checked separately; see `VALIDATION-v0.9.md` for final results and limits.
