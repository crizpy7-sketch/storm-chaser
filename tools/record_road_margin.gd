...
func _process(dt: float) -> bool:
	...
	if segment==0:
		var axis:=clampf((sin(clock*1.8)*.42-game.player_x)*3.5-game.velocity_x*.55-game.route.drift,-1.0,1.0)
	...
	if clock>1.0:shot("Road-1")
	if clock>3.0:shot("Road-3")
	if clock>5.0:shot("Road-5")
	if clock>9.0:shot("Road-Air")
	if clock>13:shot("Road-Tornado")
	if clock>15:
		var f=FileAccess.open(OS.get_environment("STORM_CAPTURE_DIR")+"/road-margin-capture.json",FileAccess.WRITE)
		...
		print("ROAD_MARGIN_CAPTURE_COMPLETE ",poses)
		quit(0)
	return false
