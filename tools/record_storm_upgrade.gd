...
func _process(dt: float) -> bool:
	...
	if pose!="straight":shot(pose)
	if clock>22:shot("Tornado")
	if clock>27:
		var f=FileAccess.open(OS.get_environment("STORM_CAPTURE_DIR")+"/storm-upgrade-capture.json",FileAccess.WRITE)
		...
		print("STORM_UPGRADE_CAPTURE_COMPLETE ",poses)
		quit(0)
	return false
