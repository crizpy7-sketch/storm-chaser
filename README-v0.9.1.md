# Storm Chaser v0.9.1 — Original Truck Style

The detailed orange pickup from v0.8.1 is back, with the smooth glide and v0.9 gameplay upgrades retained.
...
- The original detailed orange pickup artwork and its fixed depth mapping are restored. Smooth yaw remains capped at 7.5 degrees; damped suspension moves the truck and Mateo together. Steering does not warp the mesh or texture. The solid pickup supplies the road shadow; the artwork has its original wheels and roof equipment rather than separately animated geometry.
...
94 automated checks cover driving, safe checkpoint returns, retry snapshots, both complete mission paths, gallery state, persistent settings, assistance, truck appearance and suspension behavior, water pooling, and character reaction cooldowns. Rendering and exported Linux startup are checked separately; see `VALIDATION-v0.9.1.md` for final results and limits.
